...
## Trained model pipeline (model-as-data)

Alongside the analytic engine, a "model-as-data" pipeline trains win-probability models offline and serves them from committed JSON artifacts (no always-on Python):

- **Feature store:** Supabase Postgres (`server/db/schema.sql`) — the live tick loop ingests game, odds, and feature snapshots (`server/ingest/`), and `npm run backfill:live` reconstructs per-at-bat states from historical play-by-play for training on past results.
- **Training:** `python ml/train_win_prob.py` (or the weekly GitHub Actions job in `.github/workflows/train.yml`) fits scikit-learn logistic regressions and writes `models/win_prob_latest.json` (pregame) and `models/win_prob_live_latest.json` (live).
- **Inference:** Node scores artifacts in-process (`server/model/predict.ts`) with feature-skew validation; serving falls back to de-vig market probabilities until artifacts exist. Endpoints: `GET /api/model`, `GET /api/model/predict/:gamePk`, `GET /api/odds/translate`.
...
The model-as-data pipeline (Supabase feature store + offline training + JSON artifacts) is wired in: the trained live model goes live after the historical backfill and first training run; the trained pregame model accumulates labeled games via ingestion.
